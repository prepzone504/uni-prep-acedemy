...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnSend) {
    btnSend.addEventListener('click', async () => {
      ...
      try {
        const { error } = await supabaseClient.from('exam_prep_questions').insert(rowsToInsert);
        if (error) throw error;

        showToast('Questions saved successfully to the database!', 'success');
        generatedQuestions = [];
        renderExamPreview();
        btnSend.classList.remove('visible');
        ...
        btnSend.innerHTML = '💾 Save Questions';
      }
    });
  }
  ...
  window.splitIntoBlocks = function(raw) {
    ...
    let blocks = [];
    let currentBlock = [];
    const startRegex = /^\s*(?:(?:question|q|ques)\s*:?\s*\d+[\)\.\-]?|\d+\s*[\)\.\-])(?:[ \t]+|$)/i;
...
async function loadAutocompleteSuggestions() {
  try {
    const { data, error } = await supabaseClient
      .from('exam_prep_questions')
      .select('institution, faculty, department, course_code, course_title, topic')
...
Generate exactly ${count} multiple-choice exam questions.
Context: ${topic}
...
- "video_link" is optional. If none is applicable, leave it empty.
- Provide all ${count} questions. Do not truncate.`;
...
async function arrangeManualViaMistral(rawInput, modelId) {
  if (!rawInput.trim()) throw new Error('Please paste questions first.');
  const prompt = `You are a formatting assistant. Convert the user's pasted exam questions into structured JSON.
...
- Return ONLY a JSON array with NO markdown, NO code fences -- pure JSON only.
...
- "correct_answer" MUST be a single letter: A, B, C, or D.
- "video_link" is optional. Extract it if the user provided one, otherwise leave empty.
- Map the options exactly to A, B, C, D.`;
...
function parseAndAppendQuestions(rawText) {
  ...
  if (!Array.isArray(parsed) || !parsed.length) {
    throw new Error('AI returned empty or invalid question format.');
  }

  // Ensure robust properties
  const validated = parsed.map(item => ({
    question: item.question || 'Missing question',
    option_a: item.option_a || 'Missing option A',
    option_b: item.option_b || 'Missing option B',
    option_c: item.option_c || 'Missing option C',
    option_d: item.option_d || 'Missing option D',
    correct_answer: (item.correct_answer || 'A').toUpperCase().charAt(0),
    explanation: item.explanation || 'No explanation provided.',
    video_link: item.video_link || ''
  }));

  generatedQuestions = generatedQuestions.concat(validated);
}
...
function renderExamPreview() {
  ...
  previewList.innerHTML = generatedQuestions.map((q, i) => {
    ...
    return `
      <div class="preview-card" style="animation-delay: ${i * 0.02}s">
        <div class="preview-card-header">
          ...
          <span class="preview-card-diff diff-medium">Medium</span>
        </div>
        <p class="preview-card-q">${escapeHtml(q.question)}</p>
        <div class="preview-options">
          <div class="preview-opt ${isA}">A. ${escapeHtml(q.option_a)}</div>
          <div class="preview-opt ${isB}">B. ${escapeHtml(q.option_b)}</div>
          <div class="preview-opt ${isC}">C. ${escapeHtml(q.option_c)}</div>
          <div class="preview-opt ${isD}">D. ${escapeHtml(q.option_d)}</div>
        </div>
        ${q.explanation && q.explanation !== 'No explanation provided.' ? `<div style="margin-top:10px; font-size:13px; color:#666; padding:8px; background:#f5f5f5; border-radius:6px;"><strong>Explanation:</strong> ${escapeHtml(q.explanation)}</div>` : ''}
        ${q.video_link ? `<div style="margin-top:5px; font-size:13px; color:#3b82f6;"><strong>Video Link:</strong> <a href="${escapeHtml(q.video_link)}" target="_blank" style="color:#3b82f6; text-decoration:underline;">${escapeHtml(q.video_link)}</a></div>` : ''}
      </div>
    `;
  }).join('');
}
